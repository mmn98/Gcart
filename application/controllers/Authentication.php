...
class Authentication extends My_Controller
{
	...
	public function forgot_password()
	{
		...
		if ($this->input->post())
		{
			...
			{
				set_alert('error', 'your_account_is_not_active');
			}
			...
			{
				set_alert('error', 'incorrect_email');
			}
			elseif ($success == true)
			{
				set_alert('success', 'check_email_for_resetting_password');
			}
			else
			{
				set_alert('error', 'error_setting_new_password_key');
			}
	...
	public function reset_password($user_id = 0, $new_pass_key = '')
	{
		...
		$this->set_page_title('Reset_Password');

		if (!$this->Authentication_model->can_reset_password($user_id, $new_pass_key))
		{
			set_alert('error', 'password_reset_key_expired');
			redirect(site_url('authentication'));
		}
		...
		if ($this->input->post())
		{
			...
			elseif ($success == true)
			{
				set_alert('success', 'password_reset_message');
				log_activity('User Resetted the Password', $user_id);
			}
			else
			{
				set_alert('error', 'new_password_is_same_as_old_password');
